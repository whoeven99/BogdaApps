...
const MonitorTable: React.FC = () => {
    ...
    try {
      ...
      const formattedData = Object.entries(response || {}).map(([key, value]: [string, any]) => {
        ...
        return {
          ...
          estimatedCredits: value.estimatedCredits ?? 0,
          estimatedMinutes: value.estimatedMinutes != null ? Math.round(value.estimatedMinutes / 60) : 0,
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (num: number) => num != null ? `${num}分钟` : '-',
      width: 120,
    },
    {
      title: 'init时间',
